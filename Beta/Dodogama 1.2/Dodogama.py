...
class PET():
    def __init__(self,obj):
        ...
        self.pos = [self.start_x,self.max[1]-180]
        self.speed = self.max[0]//200
        self.previous = None
    def move(self):
        #print("move")
        self.pos[1] = self.max[1]-180
    ...
    def update(self):
        if len(self.img_q) == 0:
            c = r.choice([1,1,1,2,3,3,4,4,5,5])
            if c == 1:
                self.move()
            if c == 2:
                self.sleep()
...
root.geometry(gen_coord((pet.start_x,pet.max[1]-180)))
